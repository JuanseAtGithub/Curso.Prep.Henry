...
function fizzBuzz(numero) {
  ...
  let ret='';
  if(esEntero(numero/3)){
    ret='fizz';
  }
  if(esEntero(numero/5)){
    ret=ret+'buzz';
  }
  if(ret.length===0){
    return(numero);
...
function esPrimo(numero) {
  ...
  if(numero>1){
    for (let contador=1; contador<=numero; contador++){
      if(esEntero(numero/contador)){
        cantidadDeDivisores++;
      }
    }
  }
  if(cantidadDeDivisores>2 || numero<2){
    return(false);
  }  
  return(true);
}
...
function tablaDelSeis(){
  ...
  //Escribe tu código aquí
  let multiplosDeSeis=[0];
  let contador=1;
    while(multiplosDeSeis[multiplosDeSeis.length-1]<60){
      multiplosDeSeis.push(6*contador);
      contador++;
